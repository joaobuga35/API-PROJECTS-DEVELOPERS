...
export const createProject = async (
  ...
  try {
    ...
    const verifyKeys: boolean = realKeys.every((elem: string) =>
      projectsKeys.includes(elem)
    );

    if (!verifyKeys) {
      return resp.status(400).json({
        message:
          "Missing required keys: name, description, estimatedTime, repository, startDate, developerId",
      });
    }
    ...
    const queryString: string = format(
      `
                ...
                RETURNING*;
            `,
      Object.keys(newProjectsData),
      Object.values(newProjectsData)
    );
...
export const readOneProjectWithId = async (
  req: Request,
  resp: Response
): Promise<Response> => {
...
export const editProject = async (
  ...
  try {
    ...
    for (var key in newProjectBody) {
      if (
        newProjectBody[key as keyof iProjects] === null ||
        newProjectBody[key as keyof iProjects] === undefined
      ) {
    ...
    const queryString: string = format(
      `
                ...
                RETURNING*;
             `,
      Object.keys(newProjectBody),
      Object.values(newProjectBody)
    );
