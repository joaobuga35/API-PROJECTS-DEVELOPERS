...
export const createDeveloperExtraInformation = async (
  req: Request,
  resp: Response
): Promise<Response> => {
  try {
    ...
    if (
      req.body.preferredOS !== "Windows" &&
      req.body.preferredOS !== "Linux" &&
      req.body.preferredOS !== "MacOs"
    ) {
    ...
    let queryString: string = format(
      `
                ...
                RETURNING*;
            `,
      Object.keys(newDevInfosBody),
      Object.values(newDevInfosBody)
    );
  ...
  try {
    ...
    const queryString: string = format(
      `
                ...
                RETURNING *;
        `,
      Object.keys(newDeveloperInfoData),
      Object.values(newDeveloperInfoData)
    );
